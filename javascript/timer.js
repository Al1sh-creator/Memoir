...
if (!currentUser) window.location.href = "landing.html";
...
let totalTime = (parseInt(localStorage.getItem(`memoir_focus_duration_${userId}`)) || 25) * 60;
...
function calculateFocusScore() {
  // Calculate total minutes (capped at minimum 10 to avoid huge penalties for very short tests)
  // elapsedSeconds is for Session Mode, (totalTime - remaining) is for Timer Mode
  const durationSec = sessionMode ? elapsedSeconds : (totalTime - remaining);
  const minutes = Math.max(10, durationSec / 60);

  // Standardize penalties based on a 25-minute Pomodoro
  // Longer sessions = lower penalty multiplier
  // Shorter sessions = higher penalty multiplier
  const timeMultiplier = 25 / minutes;

  // Base Penalties (for a standard 25m session)
...
if (saveBtn) {
  saveBtn.onclick = () => {
    ...
    localStorage.setItem(`memoir_sessions_${userId}`, JSON.stringify(sessions));
