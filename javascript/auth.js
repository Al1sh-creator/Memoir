...
class AuthManager {
    ...
    // Migrate legacy data to new user-specific format
    migrateLegacyData(userId) {
        const legacyKeys = {
            'sessions': `memoir_sessions_${userId}`,
            'earnedBadges': `memoir_badges_${userId}`,
            'memoir_focus_duration': `memoir_focus_duration_${userId}`,
            'memoir_goal_daily': `memoir_goal_daily_${userId}`,
            'memoir_goal_weekly': `memoir_goal_weekly_${userId}`,
            'memoir_goal_monthly': `memoir_goal_monthly_${userId}`,
            'memoir_goal_total': `memoir_goal_total_${userId}`
        };
        ...
        Object.entries(legacyKeys).forEach(([oldKey, newKey]) => {
            ...
            if (data && !localStorage.getItem(newKey)) {
                ...
                // We keep old data for safety, but the app will now prefer the new key
            }
        });

        // Migrate calendar data (notes, status, tasks)
        // These keys were date/week/month specific, we append userId
        for (let i = 0; i < localStorage.length; i++) {
            const key = localStorage.key(i);
            if (key.startsWith('note_') || key.startsWith('status_') ||
                key.startsWith('weekly_focus_') || key.startsWith('month_tasks_')) {

                const newKey = `memoir_${key}_${userId}`;
