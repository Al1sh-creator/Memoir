...
const noteModal = (document.getElementById('noteModal')) ? new bootstrap.Modal(document.getElementById('noteModal')) : null;
...
function renderCalendar() {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        ...
        const hasNote = !!localStorage.getItem(`memoir_note_${dateKey}_${userId}`);
        const status = localStorage.getItem(`memoir_status_${dateKey}_${userId}`) || 'none';
...
function openNote(y, m, d) {
    // Select a day and update right-hand summary panel
    ...
    document.getElementById('noteInput').value = localStorage.getItem(`memoir_note_${selectedDateKey}_${userId}`) || '';

    // Set status selection UI for modal (kept for edit action)
    const currentStatus = localStorage.getItem(`memoir_status_${selectedDateKey}_${userId}`) || 'none';
...
function updateDailySummary(dateKey) {
    ...
    const sessions = (window.getSessions && typeof window.getSessions === 'function') ? window.getSessions() : JSON.parse(localStorage.getItem(`memoir_sessions_${userId}`) || '[]');
    ...
    // notes
    const note = localStorage.getItem(`memoir_note_${dateKey}_${userId}`) || '';
...
function loadWeeklyFocus() {
    ...
    const raw = localStorage.getItem(`memoir_weekly_focus_${key}_${userId}`);
    try { return raw ? JSON.parse(raw) : []; } catch (e) { return []; }
}

function saveWeeklyFocus(items) {
    const key = getWeekKey(new Date());
    localStorage.setItem(`memoir_weekly_focus_${key}_${userId}`, JSON.stringify(items));
}
...
function loadMonthTasks(y, m) {
    const raw = localStorage.getItem(`memoir_month_tasks_${getMonthKey(y, m)}_${userId}`);
    try { return raw ? JSON.parse(raw) : []; } catch (e) { return []; }
}
function saveMonthTasks(y, m, items) {
    localStorage.setItem(`memoir_month_tasks_${getMonthKey(y, m)}_${userId}`, JSON.stringify(items));
}
...
function saveNote() {
    ...
    if (text.trim()) localStorage.setItem(`memoir_note_${selectedDateKey}_${userId}`, text);
    else localStorage.removeItem(`memoir_note_${selectedDateKey}_${userId}`);
    ...
    if (status && status !== 'none') localStorage.setItem(`memoir_status_${selectedDateKey}_${userId}`, status);
    else localStorage.removeItem(`memoir_status_${selectedDateKey}_${userId}`);
